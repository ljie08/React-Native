...
@interface MyCityTableViewController ()<MyCityGroupCellDelegate, UISearchBarDelegate, CLLocationManagerDelegate>
...
@implementation MyCityTableViewController
...
-(NSMutableArray *) cityDatas {
    if (_cityDatas == nil) {
        ...
        for (NSDictionary *groupDic in array) {
            MyCityGroup *group = [[MyCityGroup alloc] init];
...
- (NSMutableArray *) localCityData {
    if (_localCityData == nil) {
        ...
        if (self.locationCityID != nil) {
            MyCity *city = nil;
            for (MyCity *item in self.recordCityData) {
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    MyCityGroup *group = [self.cityDatas objectAtIndex:section - 3];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.isSearch) {
        ...
        MyCity *city = [self.searchCities objectAtIndex:indexPath.row];
        [cell.textLabel setText:city.cityName];
        return cell;
    }
    
    if (indexPath.section < 3) {
        MyCityCell *cell = [tableView dequeueReusableCellWithIdentifier:cityGroupCell];
    ...
    UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:cityCell];
    MyCityGroup *group = [self.cityDatas objectAtIndex:indexPath.section - 3];
    MyCity *city =  [group.arrayCitys objectAtIndex:indexPath.row];
    
    NSString *name = [LJUtil currentLanguageWithMyChooseCity:city];
...
- (UIView *) tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    MyCityHeaderView *headerView = [tableView dequeueReusableHeaderFooterViewWithIdentifier:cityHeaderView];
...
- (CGFloat) tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        return [MyCityCell getCellHeightOfCityArray:self.localCityData];
    } else if (indexPath.section == 1) {
        return [MyCityCell getCellHeightOfCityArray:self.commonCityData];
    } else if (indexPath.section == 2){
        return [MyCityCell getCellHeightOfCityArray:self.hotCityData];
    }
    return 44.0f;
}
...
- (void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString *)searchText {
    ...
    if (searchText.length == 0) {
        ...
        self.isSearch = YES;
        for (MyCity *city in self.recordCityData) {
...
- (void)didSelctedCity:(MyCity *)city {
    ...
    [geocoder reverseGeocodeLocation:currentLocation completionHandler:^(NSArray *array, NSError *error) {
         if (array.count >0) {
             ...
             if (self.localCityData.count <= 0) {
                 ...
                 city.pinyin = currCity;
                 city.initials = [currCity substringWithRange:NSMakeRange(0, 1)];
...
- (void)initUIView {
    ...
    [self.tableView registerClass:[MyCityCell class] forCellReuseIdentifier:cityGroupCell];
    [self.tableView registerClass:[MyCityHeaderView class] forHeaderFooterViewReuseIdentifier:cityHeaderView];
}
